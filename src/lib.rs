...
impl Merkle {
    ...
    pub fn get_with_escape(&self, index: u64, data: &mut [u64], pad: bool, escape: usize) -> u64 {
        let mut hash = [0; 4];
        unsafe {
            merkle_address(index);
            ...
            let len = merkle_fetch_data();
            if len>0 {
                require(len <= data.len() as u64 && len as usize >= escape);
                for i in 0..len {
                    data[i as usize] = merkle_fetch_data();
                }

                // FIXME: avoid copy here
                let hash_check = PoseidonHasher::hash(&data[0..len as usize - escape], pad);
                require(hash[0] == hash_check[0]);
                require(hash[1] == hash_check[1]);
                require(hash[2] == hash_check[2]);
                require(hash[3] == hash_check[3]);
    ...
    pub fn set_with_escape(&mut self, index: u64, data: &[u64], pad: bool, escape: usize) {
        // place a dummy get for merkle proof convension
        unsafe {
            merkle_address(index);
        ...
        unsafe {
            ...
            require(data.len() > escape);

            let hash = PoseidonHasher::hash(&data[..data.len() - escape], pad);
            merkle_set(hash[0]);
            merkle_set(hash[1]);
            merkle_set(hash[2]);
            merkle_set(hash[3]);
